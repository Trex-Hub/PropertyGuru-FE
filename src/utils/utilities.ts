...
export const useKeyDownHandler = (action: () => void) => {
  return useCallback(
    (e: React.KeyboardEvent) => {
      if (e.key === 'Enter') {
        action && action(); // Execute the passed action
      }
    },
    [action]
  ); // Dependency on the action to ensure the handler updates when action changes
};

export const normalizeWhitespace = (text: string): string => {
  return text
    .replace(/\s+/g, ' ') // Replace any whitespace (including line breaks) with a single space
    .trim(); // Trim any leading and trailing whitespace
};

export const convertToSingleDiv = (description: any): string => {
  const combinedText = description
    ?.map((paragraph: any) => {
      const children = paragraph.children
        .map((child: any) => child.text)
        .join(' '); // Join with a space to prevent words from sticking together
