...
const FloorPlanModal: React.FC<FloorPlanModalProps> = ({
  ...
  const getPriceDisplay = (
    price: string | number,
    unitsAvailable: string | number | null | undefined
  ) => {
    const numPrice = Number(price);
    const numUnits = Number(unitsAvailable);

    if (!isNaN(numPrice) && numPrice > 0) {
      let availabilityText;
      if (!isNaN(numUnits) && numUnits > 0) {
        availabilityText = `Only ${numUnits} available`;
      } else if (unitsAvailable === 'Available') {
        availabilityText = 'Available';
      } else {
        availabilityText = 'Contact for details';
      }
      return {
        isNumeric: true,
        formattedPrice: formatPriceWithCommas(numPrice),
        availabilityText,
      };
    } else {
      let availabilityText;
      ...
      return {
        isNumeric: false,
        formattedPrice: price as string,
        availabilityText,
      };
    }
  };
              ...
              <div
                ...
                <div className='flex items-start p-4'>
                  <RadioGroup
                    ...
                    onValueChange={value =>
                      handleFloorPlanSelect(
                        floorPlan.find((p: { id: string }) => p.id === value)
                      )
                    }>
                  ...
                  <div className='flex flex-col'>
                    <div className='flex gap-1'>
                      {getPriceDisplay(plan.price, plan.unitsAvailable)
                        .isNumeric && (
