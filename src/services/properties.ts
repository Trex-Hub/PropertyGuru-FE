...
import { SEARCH_PROPERTIES_ITEMS_PER_PAGE } from '@/utils/constants';
...
export const getAllProperties = async (
  ...
  try {
    ...
    if (minPrice !== undefined && maxPrice !== undefined) {
      if (Number(maxPrice) === Infinity) {
        filters.push(`filters[$and][0][price][$gte]=${minPrice}`);
      } else {
        filters.push(
      ...
      if (Array.isArray(bedroom)) {
        const bedroomFilter = bedroom
          .map((num, index) => {
            ...
            if (num === 5) {
              return `filters[$and][6][$or][${index}][floorPlan][bedroom][$gte]=${num}`;
            }
            return `filters[$and][6][$or][${index}][floorPlan][bedroom][$eq]=${num}`;
          })
          .join('&');
    ...
    if (bathroom) {
      if (Array.isArray(bathroom)) {
        const bathroomFilter = bathroom
          .map((num, index) => {
            if (num === 5) {
              `filters[$and][7][$or][${index}][floorPlan][bathroom][$gte]=${num}`;
            } else {
...
export const getWishlistProperties = async (
  token: string,
  page = INITIAL_PAGE,
  pageSize = SEARCH_PROPERTIES_ITEMS_PER_PAGE
) => {
  try {
    const url = `${endpoints.GET_WISHLIST_PROPERTIES}?pagination[page]=${page}&pagination[pageSize]=${pageSize}`;
    const authToken = token;
    if (!authToken) {
      throw new Error('No JWT token provided. User might need to log in.');
    }
    ...
    return formatResponse(response.data, ResponseStatusEnum.SUCCESS);
  } catch (err) {
    logger.error('Error in getWishlistProperties', err);
