...
const TopLabelClient = ({
  ...
  return (
    <div className='md:flex justify-between p-2 py-4 md:p-4'>
      ...
      <div className='flex justify-between gap-3 items-top md:max-h-[2.875rem] items-center'>
        <Button
        ...
        <div className='flex flex-row justify-between'>
          {attributes?.developer?.data?.attributes?.logo?.data?.attributes
            ?.url && (
            <Image
              src={`${NEXT_PUBLIC_ASSETS_URL}${attributes?.developer?.data?.attributes?.logo?.data?.attributes.url}`}
              width={120}
              height={20}
              sizes='100vw'
              className='w-20 md:hidden mr-5'
              alt={
                attributes?.developer?.data?.attributes?.logo?.data?.attributes
