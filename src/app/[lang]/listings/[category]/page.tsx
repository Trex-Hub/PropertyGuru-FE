import { Suspense } from 'react';
import { FiChevronRight } from 'react-icons/fi';
import Script from 'next/script';
import Link from 'next/link';
import PropertyDetailForm from '@/components/PropertyDetailForm';
import {
  FeaturedPropertiesLoader,
  GoogleAdVideoLoader,
} from '@/components/Loading';
import SectionFeaturedProperties from '@/components/SideBar/SectionFeaturedProperties';
import PropertyDetailFormButton from '@/components/PropertyDetailFormButton';
import {
  SearchBarLoader,
  SearchResultLoader,
} from '../../advanced-search/_components/Loaders';
import SearchBar from '../../advanced-search/_sections/SearchBar';
import SearchBarResult from '../../advanced-search/_sections/SearchBarResult';
import { capitalize } from '@/utils/utilities';
import GoogleAdVideo from '@/components/GoogleAdVideo';
import AdContainer from '@/components/AdContainer';

export const generateMetadata = ({
  params,
}: {
  params: { category: string; lang: string };
}) => {
  const baseUrl = process.env.BASE_FE_URL;
  return {
    title: `${capitalize(params.category)} Archives | Propertyguru`,
    description: `${capitalize(params.category)}`,
    robots: {
      index: true,
      follow: true,
      'max-image-preview': 'large',
      'max-snippet': -1,
      'max-video-preview': -1,
    },
    alternates: {
      canonical: `${params?.lang}/listings/${params.category}`,
    },
    openGraph: {
      images: ['/images/home/logo.png'],
      url: `${baseUrl}/${params?.lang}/listings/${params.category}`,
      title: `${capitalize(params.category)} Archives`,
      locale: 'en_AE',
      type: 'website',
    },
    twitter: {
      images: ['/images/home/logo.png'],
      title: `${capitalize(params.category)} Archives`,
    },
  };
};

type ListingCategoriesPropertiesParams = {
  searchParams: {
    minPrice: number;
    maxPrice: number;
    city: string;
    area: string;
    developer: string;
    propertyType: string;
    readiness: string;
    minArea: number;
    maxArea: number;
    bedroom: number | number[];
    bathroom: number | number[];
    possession: string | string[];
    amenity: string | string[];
    keyword: string;
    page: string;
    sort: string;
  };
  params: {
    lang: string;
    category: string;
  };
};

export default async function ListingCategoriesProperties({
  searchParams,
  params,
}: ListingCategoriesPropertiesParams) {
  const listingCategoriesPropertiesJsonLd = {
    '@context': 'https://schema.org',
    '@graph': [
      {
        '@type': 'CollectionPage',
        '@id': `${process.env.BASE_FE_URL}/${params?.lang}/listings/${params.category}`,
        url: `${process.env.BASE_FE_URL}/${params?.lang}/listings/${params.category}`,
        name: `${capitalize(params.category)} Archives | Propertyguru`,
        isPartOf: { '@id': `${process.env.BASE_FE_URL}/#website` },
        primaryImageOfPage: {
          '@id': `${process.env.BASE_FE_URL}/${params?.lang}/listings/${params.category}/#primaryimage`,
        },
        image: {
          '@id': `${process.env.BASE_FE_URL}/${params?.lang}/listings/${params.category}/#primaryimage`,
        },
        thumbnailUrl: `${process.env.BASE_FE_URL}/${params?.lang}/images/home/listing-categories-properties.jpg`,
        breadcrumb: {
          '@id': `${process.env.BASE_FE_URL}/${params?.lang}/listings/${params.category}/#breadcrumb`,
        },
        inLanguage: 'en-US',
      },
      {
        '@type': 'ImageObject',
        inLanguage: 'en-US',
        '@id': `${process.env.BASE_FE_URL}/${params?.lang}/listings/${params.category}/#primaryimage`,
        url: `${process.env.BASE_FE_URL}/${params?.lang}/images/home/listing-categories-properties.jpg`,
        contentUrl: `${process.env.BASE_FE_URL}/${params?.lang}/images/home/listing-categories-properties.jpg`,
        width: 1000,
        height: 600,
      },
      {
        '@type': 'BreadcrumbList',
        '@id': `${process.env.BASE_FE_URL}/${params?.lang}/listings/${params.category}/#breadcrumb`,
        itemListElement: [
          {
            '@type': 'ListItem',
            position: 1,
            name: 'Home',
            item: `${process.env.BASE_FE_URL}/${params?.lang}`,
          },
          { '@type': 'ListItem', position: 2, name: `${params.category}` },
        ],
      },
      {
        '@type': 'WebSite',
        '@id': `${process.env.BASE_FE_URL}/${params?.lang}/#website`,
        url: `${process.env.BASE_FE_URL}/${params?.lang}`,
        name: 'Propertyguru.ae | Buy Properties in Dubai',
        description:
          'Your only platform for property investments in Dubai and the UAE!',
        publisher: {
          '@id': `${process.env.BASE_FE_URL}/${params?.lang}/#organization`,
        },
        alternateName: 'propertyguru',
        potentialAction: [
          {
            '@type': 'SearchAction',
            target: {
              '@type': 'EntryPoint',
              urlTemplate: `${process.env.BASE_FE_URL}/${params?.lang}/advanced-search?city={city}&area={area}&developer={developer}&propertyType={propertyType}&minPrice={minPrice}&maxPrice={maxPrice}&readiness={readiness}&page={page}`,
            },
            'query-input': [
              'name=city',
              'name=area',
              'name=developer',
              'name=propertyType',
              'name=minPrice',
              'name=maxPrice',
              'name=readiness',
              'name=page',
            ],
          },
        ],
        inLanguage: 'en-US',
      },
      {
        '@type': 'Organization',
        '@id': `${process.env.BASE_FE_URL}/${params?.lang}/#organization`,
        name: 'propertyguru.ae',
        alternateName: 'propertyguru',
        url: `${process.env.BASE_FE_URL}/${params?.lang}`,
        logo: {
          '@type': 'ImageObject',
          inLanguage: 'en-US',
          '@id': `${process.env.BASE_FE_URL}/${params?.lang}/#/schema/logo/image/`,
          url: `${process.env.BASE_FE_URL}/${params?.lang}/images/home/listing-categories-properties.jpg`,
          contentUrl: `${process.env.BASE_FE_URL}/${params?.lang}/images/home/listing-categories-properties.jpg`,
          width: 500,
          height: 500,
          caption: 'propertyguru.ae',
        },
        image: {
          '@id': `${process.env.BASE_FE_URL}/${params?.lang}/#/schema/logo/image/`,
        },
      },
    ],
  };

  return (
    <div className='md:px-20 px-0'>
      <Script id='page-action-schema' type='application/ld+json'>
        {JSON.stringify(listingCategoriesPropertiesJsonLd)}
      </Script>
      <div className='flex items-center px-4 md:px-6 font-primary md:py-4 mt-2 py-2'>
        <Link href='/'>
          <p className='text-[0.625rem] md:text-xs  text-primary-secondaryTextColor'>
            Home
          </p>
        </Link>
        <FiChevronRight className='text-primary-textGrayColor' />
        <p className='text-[0.625rem] md:text-xs text-primary-titleTextColor capitalize'>
          {params.category}
        </p>
      </div>
      <Suspense fallback={<SearchBarLoader />}>
        <SearchBar searchParams={searchParams} params={params} />
      </Suspense>
      <div className='lg:flex lg:flex-row flex-col w-full h-full gap-2'>
        <Suspense fallback={<SearchResultLoader />}>
          <SearchBarResult searchParams={searchParams} params={params} />
        </Suspense>
        <div className='flex flex-col gap-5 items-center justify-center md:hidden lg:hidden'>
          <AdContainer
            slotId='div-gpt-ad-1727175348445-0'
            slotName='/22718860182/pg-widget-right1'
            minWidth='300'
            minHeight='250'
          />
          <Suspense fallback={<GoogleAdVideoLoader />}>
            <GoogleAdVideo />
          </Suspense>
        </div>
        <div className='lg:w-[32%] py-4 lg:p-4 flex flex-col gap-5 lg:top-0 h-full'>
          <div className='hidden lg:block'>
            <div className='gap-5 flex flex-col'>
              <AdContainer
                slotId='div-gpt-ad-1685538479815-0'
                slotName='/22718860182/pg-widget-right2'
                minWidth='300'
                minHeight='250'
              />
              <PropertyDetailForm />
              <PropertyDetailFormButton />
            </div>
            <Suspense fallback={<GoogleAdVideoLoader />}>
              <GoogleAdVideo />
            </Suspense>
          </div>
          <Suspense fallback={<FeaturedPropertiesLoader />}>
            <SectionFeaturedProperties />
          </Suspense>
        </div>
      </div>
    </div>
  );
}
